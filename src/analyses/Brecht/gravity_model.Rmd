...
```{r}
setwd("/Users/brecht/Documents/citynet_2")
nodes <- read_delim("citynet/src/input/nodes.csv",
                         ";", escape_double = FALSE, 
                         col_types = 
...
nodes <- tibble(nodes)
nodes <- select(nodes, -c(X1, Mua, Mua_en, Mua_fr))
head(nodes)
```
...
```
```{r}
ggplot(nodes, aes(x=log(en_degree_prop), y=log(fr_degree_prop), shape=lang)) +
...
degree_cor <-
  ...
  xlim(-5, 3.5) +
  ylim(-5, 3.5) +
  xlab("Proportional degree centrality in the English network (log)")+
  ylab("Proportional degree centrality in the French network (log)")+
...
```{r}
lm.fit <- lm(col_en ~ POP_A + POP_B + distance, edges_nz)
summary(lm.fit)
```
...
```{r}
glm.fit <- glm(log(col_fr) ~ POP_A + POP_B + distance, 
               data   = nooutliers_df, 
               family = poisson)
```
