...
```{r}
# self focus ratio
CL1 <- subset(nodes, lang == "FR", c("fr_degree")) %>% 
  max()

CLn1 <-subset(nodes, lang != "FR", c("fr_degree")) %>% 
  max()
...
```{r}
# self focus ratio
CL1 <- subset(nodes, lang == "EN", c("en_degree")) %>% 
  max()

CLn1 <-subset(nodes, lang != "EN", c("en_degree")) %>% 
  max()
  

SFR_english <- CL1 / CLn1
```
